<template>
    <div v-if="selectedResult" class="mt-6 w-full text-center">
        ...
        <div class="border border-gray-400 mt-4 w-full">
            <a :href="selectedResult.html_url" target="_blank">{{ selectedResult.html_url }}</a>
            <h3><strong>Forks:</strong> {{ selectedResult.forks_count }}</h3>
            <h3><strong>Stargazers:</strong> {{ selectedResult.stargazers_count }}</h3>
            <h3><strong>Open Issues:</strong> {{ selectedResult.open_issues_count }}</h3>
            ...
            <div v-if="showmoreDetails" class="mt-4 flex flex-col justify-center items-center">
                ...
                <ul class="list-disc pl-8 py-2">
                    ...
                    <h3><strong>Created:</strong> {{ formatDate(selectedResult.created_at) }}</h3>
                    <h3><strong>Last Updated:</strong> {{ formatDate(selectedResult.updated_at) }}</h3>
                </ul>
            </div>
        </div>
    </div>
